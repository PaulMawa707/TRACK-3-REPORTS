...
def create_excel_file(utilization, df2, merged_df, trips_df2, group_name, prev_month_data=None):
    ...
    # --- FIX: ensure Advanced Score is simple sum of all violations ---
    exclude_cols = {
        "Grouping", "Group", "Total Distance (km)",
        "Previous Advanced Score", "Advanced Score", "Advanced Score Change"
    }
    # recompute Advanced Score cleanly
    violation_cols = [c for c in merged_df.columns if c not in exclude_cols]
    merged_df[violation_cols] = merged_df[violation_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
    merged_df["Advanced Score"] = merged_df[violation_cols].sum(axis=1).round(2)

    # recompute Previous Advanced Score if prev_month_data provided
    if prev_month_data is not None and not prev_month_data.empty:
        prev_exclude = {
            "Grouping", "Group", "Total Distance (km)",
            "Previous Advanced Score", "Advanced Score", "Advanced Score Change"
        }
        prev_violation_cols = [c for c in prev_month_data.columns if c not in prev_exclude]
        prev_month_data[prev_violation_cols] = prev_month_data[prev_violation_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
        prev_month_data["Previous Advanced Score"] = prev_month_data[prev_violation_cols].sum(axis=1).round(2)

        # merge and compute change
        merged_df = merged_df.merge(
            prev_month_data[["Grouping", "Previous Advanced Score"]],
            on="Grouping",
            how="left"
        ).fillna(0)
