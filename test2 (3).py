    ...
    if st.button("🚀 Fetch Monthly Eco Driving Reports"):
        try:
            ...
            if not trip_df.empty:
                ...
                download_single_df_excel(trips_df2, "Trips", f"{group_name}_Trips")
                st.subheader("Utilization Report")
                st.dataframe(utilization)
                download_single_df_excel(utilization, "Utilization", f"{group_name}_Utilization")
            else:
            ...
            st.dataframe(df2)
            download_single_df_excel(df2, "EcoDriving", f"{group_name}_EcoDriving")

            st.subheader("RAG Score Report")
            st.dataframe(merged_df)
            download_single_df_excel(merged_df, "RAGScore", f"{group_name}_RAGScore")
