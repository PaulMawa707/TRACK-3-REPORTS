...
def download_single_df_excel(df, sheet_name, file_name):
    df = remove_timezones(df)  # <-- ensure no tz-aware datetimes
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        df.to_excel(writer, index=False, sheet_name=sheet_name)
        ws = writer.sheets[sheet_name]
        for i, col in enumerate(df.columns):
            ws.set_column(i, i, max(df[col].astype(str).map(len).max(), len(str(col))) + 2)
    output.seek(0)
    b64 = base64.b64encode(output.read()).decode()
    href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{file_name}.xlsx">Download {sheet_name}</a>'
    st.markdown(href, unsafe_allow_html=True)
    ...
    if st.button("🚀 Fetch Monthly Eco Driving Reports"):
        try:
            ...
            if not trip_df.empty:
                ...
                trips_df2_clean = remove_timezones(trips_df2)
                download_single_df_excel(trips_df2_clean, "Trips", f"{group_name}_Trips")

                st.subheader("Utilization Report")
                st.dataframe(utilization)
                utilization_clean = remove_timezones(utilization)
                download_single_df_excel(utilization_clean, "Utilization", f"{group_name}_Utilization")
            else:
            ...
            st.dataframe(df2)
            df2_clean = remove_timezones(df2)
            download_single_df_excel(df2_clean, "EcoDriving", f"{group_name}_EcoDriving")

            st.subheader("RAG Score Report")
            st.dataframe(merged_df)
            merged_df_clean = remove_timezones(merged_df)
            download_single_df_excel(merged_df_clean, "RAGScore", f"{group_name}_RAGScore")
            ...
            if prev_month_data is not None:
                ...
                download_single_df_excel(prev_month_data, "PrevMonthEco", f"{group_name}_PrevMonthEco")
